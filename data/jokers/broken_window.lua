...
SMODS.Joker {
	...
	pos = { x = 3, y = 0 },
    cost = 1,
    config = { extra = {minCards = 7, cards = 13, selfDestruct = false,suit = "Diamonds",debuff_name = "unik_window",death_message = "k_unik_window_fixed",color = "a9a295"} },
        ...
        if G.deck and card.added_to_deck then 
            for i, w in pairs(G.deck.cards) do
                ...
                if w:is_suit(card.ability.extra.suit,true) then
                    Cards = Cards + 1
                    SMODS.debuff_card(w,true,"unik_window")
                end
            end
        end
    ...
    remove_from_deck = function(self, card, from_debuff)
        if G.deck then 
            for i, w in pairs(G.deck.cards) do
                --bypass debuff to ensure it doesnt self destruct
                if w:is_suit(card.ability.extra.suit,true) then
                    SMODS.debuff_card(w,false,"unik_window")
                end
            end
        end
        if G.hand then 
