...
SMODS.Consumable:take_ownership("c_paperback_apostle_of_wands",{
    ...
    PB_UTIL.use_consumable_animation(card, nil, function()
		if #G.jokers.cards < G.jokers.config.card_limit then
			...
			if v.discovered and v.rarity ~= 4 and v.rarity ~= 'unik_ancient' and v.rarity ~= 'cry_epic' and v.rarity ~= 'cry_exotic' and v.rarity ~="unik_legendary_blind_finity" and (not SMODS.Mods.Cryptid or not SMODS.Mods.Cryptid.can_load or (SMODS.Mods.Cryptid and SMODS.Mods.Cryptid.can_load and not Cryptid.pin_debuff[v.rarity])) and not next(SMODS.find_card(v.key)) then
