...
function Card:set_debuff(should_debuff)
    if (self.ability and self.ability.unik_shielded or (SMODS.has_enhancement(self,'m_wild'))) and not self.ability.unik_ultradebuffed  then
        if self.debuff then
            self.debuff = false
...
function SMODS.debuff_card(card, debuff, source)
    if (card.ability and card.ability.unik_shielded or SMODS.has_enhancement(card,'m_wild')) and not card.ability.unik_ultradebuffed   then
        if card.debuff then
            card.debuff = false
            -- card_eval_status_text(self, "extra", nil, nil, nil, { message = localize("k_nope_ex"),delay = 0.2})
