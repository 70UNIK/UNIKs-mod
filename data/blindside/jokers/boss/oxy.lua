BLINDSIDE.Joker({
    ...
    death_card = {
        card = UNIK.has_bos() and 'j_' .. UNIK.get_bos_prefix() .. '_oxy' or 'j_unik_blindside_taunt_oxy_pwx', 
    ...
    calculate = function(self, blind, context)
        ...
        if context.scoring_hand and context.individual and context.cardarea == G.play and not G.GAME.blind.disabled then
            if tableContains(context.other_card, context.scoring_hand) and (context.other_card):get_seal() and context.other_card.facing ~= 'back' then
                return {
                    message = "X" .. 1.3 .. localize('k_unik_jmult'),
                    colour = G.C.BLACK,
                    focus = context.other_card,
                    func = function ()
                        BLINDSIDE.change_fire_amount({amount = 2})
                        BLINDSIDE.add_fire()
                        BLINDSIDE.chipsmodifyV2({x_mult = 1.3})   
